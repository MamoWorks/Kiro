...
func (tlm *ToolLifecycleManager) HandleToolCallRequest(request ToolCallRequest) []SSEEvent {
	...
	if !tlm.textIntroGenerated && len(request.ToolCalls) > 0 {
		...
		// utils.Log("自动生成工具调用文本介绍",
		// 	utils.LogInt("intro_events", len(textIntroEvents)),
		// 	utils.LogString("first_tool", request.ToolCalls[0].Function.Name))
	}

	for _, toolCall := range request.ToolCalls {
		// 检查工具是否已存在，避免重复创建
		if existing, exists := tlm.activeTools[toolCall.ID]; exists {
			utils.Log("工具已存在，更新参数",
				utils.LogString("tool_id", toolCall.ID),
				utils.LogString("tool_name", toolCall.Function.Name),
				utils.LogString("existing_status", existing.Status.String()))

			// 解析工具调用参数
			var arguments map[string]any
			if err := utils.SafeUnmarshal([]byte(toolCall.Function.Arguments), &arguments); err != nil {
				utils.Log("解析工具调用参数失败",
					utils.LogString("tool_id", toolCall.ID),
					utils.LogString("tool_name", toolCall.Function.Name),
					utils.LogErr(err))
		...
		utils.Log("开始处理工具调用",
			utils.LogString("tool_id", toolCall.ID),
			utils.LogString("tool_name", toolCall.Function.Name),
			utils.LogInt("block_index", execution.BlockIndex))
...
func (tlm *ToolLifecycleManager) HandleToolCallResult(result ToolCallResult) []SSEEvent {
	...
	if !exists {
		utils.Log("收到未知工具调用的结果",
			utils.LogString("tool_call_id", result.ToolCallID))
		return events
	}
	...
	// utils.Log("工具调用完成",
	// 	utils.LogString("tool_id", result.ToolCallID),
	// 	utils.LogString("tool_name", execution.Name),
	// 	utils.LogInt64("execution_time", executionTime))
...
func (tlm *ToolLifecycleManager) HandleToolCallError(errorInfo ToolCallError) []SSEEvent {
	...
	if !exists {
		utils.Log("收到未知工具调用的错误",
			utils.LogString("tool_call_id", errorInfo.ToolCallID))
		return events
	}
	...
	utils.Log("工具调用失败",
		utils.LogString("tool_id", errorInfo.ToolCallID),
		utils.LogString("tool_name", execution.Name),
		utils.LogString("error", errorInfo.Error),
		utils.LogAny("execution_time", executionTime))
...
func (tlm *ToolLifecycleManager) UpdateToolArguments(toolID string, arguments map[string]any) {
	// utils.Log("更新工具调用参数",
	// 	utils.LogString("tool_id", toolID),
	// 	utils.LogAny("arguments", arguments))

	// 检查活跃工具
	if execution, exists := tlm.activeTools[toolID]; exists {
		execution.Arguments = arguments
		// utils.Log("已更新活跃工具的参数",
		// 	utils.LogString("tool_id", toolID),
		// 	utils.LogString("tool_name", execution.Name))
		return
	}
	...
	if execution, exists := tlm.completedTools[toolID]; exists {
		execution.Arguments = arguments
		// utils.Log("已更新已完成工具的参数",
		// 	utils.LogString("tool_id", toolID),
		// 	utils.LogString("tool_name", execution.Name))
		return
	}

	utils.Log("未找到要更新参数的工具",
		utils.LogString("tool_id", toolID))
}
...
func (tlm *ToolLifecycleManager) UpdateToolArgumentsFromJSON(toolID string, jsonArgs string) {
	...
	if err := utils.SafeUnmarshal([]byte(jsonArgs), &arguments); err != nil {
		utils.Log("解析工具参数JSON失败",
			utils.LogString("tool_id", toolID),
			utils.LogString("json", jsonArgs),
			utils.LogErr(err))
		return
	}
